...
def create_user(username='testuser', password=PASSWORD, email='user@example.com', is_staff=False):
...
class TrialDaysTests(APITestCase):
    ...
    def test_get_single_trial_day(self):
        ...
        url = reverse('trial_day_detail', kwargs={'pk': trial_day.id})  # Adjust based on your url name
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {self.access_token}')
        response = self.client.get(url)
