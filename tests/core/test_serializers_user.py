...
class AuthenticationTest(APITestCase):
    ...
    def test_user_can_log_in_with_email(self):
        ...
        response = self.client.post(
            reverse("log_in"), data={"username": user.email, "password": PASSWORD}
        )
...
class UserTestCase(APITestCase):
    ...
    def test_get_user(self):
        ...
        self.assertEqual(
            response.data["stripe_customer_id"], self.user.stripe_customer_id
        )
    ...
    def test_update_user(self):
        ...
        response = self.client.put(
            url, data, HTTP_AUTHORIZATION=f"Bearer {self.access}", format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
