...
class ContactSubmissionTests(APITestCase):
    ...
    def test_get_all_submissions(self):
        ...
        ContactSubmission.objects.create(name="Test1", email="test1@example.com", message="Test Message 1")
        ContactSubmission.objects.create(name="Test2", email="test2@example.com", message="Test Message 2")
    ...
    def test_get_single_submission(self):
        ...
        submission = ContactSubmission.objects.create(name="Test3", email="test3@example.com", message="Test Message 3")
        url = reverse('contact', kwargs={'pk': submission.id})  # Adjust based on your url name
        response = self.client.get(url, HTTP_AUTHORIZATION=f'Bearer {self.access_staff_user_3}')
    ...
    def test_delete_submission(self):
        ...
        submission = ContactSubmission.objects.create(name="Test4", email="test4@example.com", message="Test Message 4")
        url = reverse('contact', kwargs={'pk': submission.id})  # Adjust based on your url name
        response = self.client.delete(url, HTTP_AUTHORIZATION=f'Bearer {self.access_staff_user_3}')

        assert response.status_code == status.HTTP_204_NO_CONTENT
        assert ContactSubmission.objects.filter(pk=submission.id).exists() == False
