...
class EventViewSetTest(APITestCase):
    ...
    def test_non_staff_user_can_read_events(self):
        response = self.client.get(
            reverse("events"), HTTP_AUTHORIZATION=f"Bearer {self.access_user}"
        )  # Adjust 'event-list' to your actual URL name for EventViewSet list route
    ...
    def test_non_staff_user_cannot_create_event(self):
        ...
        response = self.client.post(
            reverse("events"),
            data=data,
            HTTP_AUTHORIZATION=f"Bearer {self.access_user}",
        )
    ...
    def test_staff_user_can_create_event(self):
        data = {
            ...
            "startTime": "12:00",
            "endTime": "13:00",
            "description": "A staff created event.",
        }
        response = self.client.post(
            reverse("events"),
            data=data,
            HTTP_AUTHORIZATION=f"Bearer {self.access_staff_user}",
        )
    ...
    def test_staff_user_can_update_event(self):
        ...
        response = self.client.patch(
            reverse("event", kwargs={"pk": event.pk}),
