...
class VideoPaginationTest(APITestCase):
    def setUp(self):
        ...
        self.updated_user = create_user(
            "testuser2", PASSWORD, True, "testuser2@test.com"
        )
    ...
    def test_video_pagination_free(self):
        ...
        response = self.client.get(
            url, HTTP_AUTHORIZATION=f"Bearer {self.access_free_user_1}"
        )
    ...
    def test_video_pagination_payed_and_staff(self):
        ...
        url = reverse("video-list") + "?page=1&page_size=10"
        response = self.client.get(
            url, HTTP_AUTHORIZATION=f"Bearer {self.access_paid_user_2}"
        )
        # print(response.data)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data["results"]), 10)
    ...
    def test_create_video_with_category(self):
        category_data = {"title": "Test Category", "description": "Test description"}
        ...
        self.assertEqual(
            response.data["categories"][0]["description"], category["description"]
        )
        # Convert actual value to datetime object in UTC
        actual_date = timezone.datetime.strptime(
            response.data["date_of_creation"], "%Y-%m-%dT%H:%M:%S.%f%z"
...
class CategoryAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.category = Category.objects.create(
            title="Existing Category", description="Existing description"
        )
    ...
    def test_create_category(self):
        ...
        response = self.client.post(
            ...
            self.category_data,
            format="json",
            HTTP_AUTHORIZATION=f"Bearer {self.access_staff_user_2}",
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_delete_category(self):
        url = reverse("category-detail", args=[self.category.id])
        response = self.client.delete(
            url, HTTP_AUTHORIZATION=f"Bearer {self.access_staff_user_2}"
        )
    ...
    def test_patch_add_video_to_category(self):
        ...
        response = self.client.patch(
            url, data, HTTP_AUTHORIZATION=f"Bearer {self.access_staff_user_2}"
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
...
class SearchVideoAPITestCase(APITestCase):
    ...
    def test_search_video_api(self):
        ...
        )  # Assuming there is only one video with the specified title
