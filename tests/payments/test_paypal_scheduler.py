...
class TestSchedulerJob(TestCase):
    def test_simple_job_creation_and_execution(self):
        ...
        run_time = datetime.utcnow() + timedelta(seconds=10)  # Schedule to run in 10 seconds
        ...
        job_execution_exists = DjangoJobExecution.objects.filter(job_id='test_simple_job').exists()
        self.assertTrue(job_execution_exists, "The job was not executed and recorded in the DjangoJobExecution table.")
