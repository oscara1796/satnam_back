...
class SubscriptionPlanAPIView(APIView):
    ...
    def deactivate_paypal_plan(self, plan_id):
        ...
        paypal_url = f"{BASE_PAYPAL_URL}/v1/billing/plans/{plan_id}/deactivate"
    ...
    def delete(self, request, pk):
        ...
        access_token = get_paypal_access_token()
        paypal_url = (
            f"{BASE_PAYPAL_URL}/v1/billing/plans/{plan.paypal_plan_id}/deactivate"
        )
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {access_token}",
...
class PaypalSubscriptionView(APIView):
    ...
    def get(self, request, subscription_id=None):
        ...
        try:
            ...
            logger.info(
                f"Successfully retrieved subscription details for ID: {subscription_id}"
            )
    ...
    def post(self, request):
        ...
        logger.info(
            f"Attempting to save  Paypal subscription ID: {subscription_id} for user ID: {user_id}"
        )
        try:
            ...
            user.save()
            logger.info(
                f"Successfully saved  paypal subscription ID: {subscription_id} for user ID: {user_id}"
            )
            return Response(
    ...
    def patch(self, request, pk):
        ...
        logger.info(
            f"Received request to partially update subscription for user ID: {pk}"
        )
        try:
            ...
            if user.paypal_subscription_id:
                ...
                if paypal_subscription["status"] == "SUSPENDED":
                    logger.info(
                        f"Subscription is suspended, attempting to reactivate for user ID: {pk}"
                    )
                    self.activate_paypal_subscription(user.paypal_subscription_id)
                    remove_scheduled_deletion(user.paypal_subscription_id)
                    logger.info(
                        f"Subscription reactivated and scheduled deletion removed for user ID: {pk}"
                    )
                    return Response(
                        {
                    ...
                    logger.warning(
                        f"Subscription for user ID: {pk} is not suspended. Cannot reactivate."
                    )
                    return Response(
            ...
            print("ERROR ", e)
            logger.error(
                f"An error occurred while updating subscription for user ID: {pk}: {e}"
            )
            return Response(
                {"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def delete(self, request, pk):
        ...
        try:
            ...
            if user.paypal_subscription_id:
                logger.info(
                    f"Attempting to deactivate PayPal subscription for user ID: {pk}"
                )
                ...
                if last_billing_date:
                    ...
                    logger.debug(
                        f"Scheduling deletion at: {formatted_time} for testing purposes."
                    )
                    ...
                    logger.info(
                        f"Scheduled deletion for PayPal subscription {user.paypal_subscription_id} for user ID: {pk}"
                    )

            logger.info(
                f"Successfully deactivated PayPal subscription for user ID: {pk}"
            )
            return Response(
            ...
            logger.error(
                f"An error occurred while deactivating subscription for user ID: {pk}: {e}"
            )
            return Response(
                {"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def deactivate_paypal_subscription(self, subscription_id):
        ...
        logger.info(
            f"Attempting to deactivate PayPal subscription ID: {subscription_id}"
        )
        ...
        try:
            ...
            logger.info(
                f"User {user.id} next  paypal billing time updated to {next_billing_time}"
            )

            response = requests.post(url, headers=headers)
            if response.status_code in (200, 204):
                logger.info(
                    f"Successfully deactivated PayPal subscription ID: {subscription_id}"
                )
                return last_billing_date
            else:
                logger.error(
                    f"Failed to deactivate PayPal subscription ID: {subscription_id} - {response.text}"
                )
                raise Exception(
            ...
            logger.error(
                f"An error occurred during PayPal subscription deactivation: {e}"
            )
            raise
    ...
    def activate_paypal_subscription(self, subscription_id):
        ...
        logger.info(
            f"Attempting to reactivate PayPal subscription ID: {subscription_id}"
        )
        access_token = get_paypal_access_token()
        ...
        try:
            ...
            if response.status_code not in (200, 204):
                logger.error(
                    f"Failed to reactivate PayPal subscription ID: {subscription_id} - {response.text}"
                )
                raise Exception(
                    f"Failed to activate PayPal subscription {subscription_id}: {response.text}"
                )
            logger.info(
                f"Successfully reactivated PayPal subscription ID: {subscription_id}"
            )
            return response.status_code == 204
        except Exception as e:
            logger.error(
                f"An error occurred during PayPal subscription reactivation: {e}"
            )
            raise
    ...
    def get_last_billing_date(self, subscription_id):
        ...
        logger.info(
            f"Fetching last billing date for PayPal subscription ID: {subscription_id}"
        )
        access_token = get_paypal_access_token()
        url = f"{BASE_PAYPAL_URL}/v1/billing/subscriptions/{subscription_id}"
        headers = {
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"Retrieved last billing date for PayPal subscription ID: {subscription_id}: {last_billing_date}"
                )
                return last_billing_date
            else:
                logger.error(
                    f"Failed to retrieve subscription details for ID: {subscription_id} - {response.text}"
                )
                raise Exception(
                    f"Failed to retrieve subscription details: {response.text}"
                )
        except Exception as e:
            logger.error(
                f"An error occurred while retrieving last billing date for PayPal subscription ID: {subscription_id}: {e}"
            )
            raise
